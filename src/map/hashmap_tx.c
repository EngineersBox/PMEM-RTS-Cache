...
PMEMoid hm_tx_remove(PMEMobjpool *pop, TOID(struct hashmap_tx) hashmap, uint64_t key) {
    ...
    uint64_t h = hash(&hashmap, &buckets, key);
    for (var = D_RO(buckets)->bucket[h]; !TOID_IS_NULL(var); prev = var, var = D_RO(var)->next) {
        if (D_RO(var)->key == key)
            break;
    }
...
int hm_tx_foreach(PMEMobjpool *pop, TOID(struct hashmap_tx) hashmap, int (*cb)(uint64_t key, PMEMoid value, void *arg), void *arg) {
    ...
    for (size_t i = 0; i < D_RO(buckets)->nbuckets; ++i) {
        ...
        for (var = D_RO(buckets)->bucket[i]; !TOID_IS_NULL(var); var = D_RO(var)->next) {
 ...
 */
static void hm_tx_debug(PMEMobjpool *pop, TOID(struct hashmap_tx) hashmap, FILE *out) {
    TOID(struct buckets) buckets = D_RO(hashmap)->buckets;
    TOID(struct entry) var;
    ...
    fprintf(
        ...
        D_RO(hashmap)->hash_fun_b,
        D_RO(hashmap)->hash_fun_p
    );
    fprintf(
        out,
        "count: %" PRIu64 ", buckets: %zu\n",
        D_RO(hashmap)->count,
...
PMEMoid hm_tx_get(PMEMobjpool *pop, TOID(struct hashmap_tx) hashmap, uint64_t key) {
    ...
    for (var = D_RO(buckets)->bucket[h]; !TOID_IS_NULL(var); var = D_RO(var)->next)
        if (D_RO(var)->key == key)
